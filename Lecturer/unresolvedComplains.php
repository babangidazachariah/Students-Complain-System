				...
				xmlhttp.onreadystatechange = function()
				{
					...
					if((xmlhttp.readyState == 4) && (xmlhttp.status == 200))
					{
						...
						if(xmlhttp.responseText.length > 0){
							...
							complain += "<hr /><div class='col-sm-12'><div class='col-sm-12'>Response<textarea name='details' id='response' class='form-control'></textarea></div><div class='col-sm-6'><br><input type='submit' onclick='Javascript:Response("+details[5]+")' name='submit' value='Send' class='btn btn-success'></div><div class='col-sm-6'><br><input type='hidden' name='complainID' value='"+details[5]+"' class='btn btn-success'></div></div>";
				...
				if(response !=""){
					...
					xmlhttp.onreadystatechange = function()
					{
						...
						if((xmlhttp.readyState == 4) && (xmlhttp.status == 200))
						{
							...
							document.forms['unresolvedComplains'].submit();
							
						}
						}
					//alert(routine);
					xmlhttp.open("GET", "../genFunctions.php?func=Response&id="+id+"&details="+response,true);
					xmlhttp.send();
				}else{
					alert("Input Response in the Textboxt");
				}
			}
		</script>
    </head>
        ...
        <div class="col-sm-10 col-sm-offset-1 thumbnail">
            ...
            <div class="col-sm-12 thumbnail">
                ...
                <div class="col-sm-12 row text-justify">
                  <p></p>
                    <fieldset>
					<form name="unresolvedComplains" id="unresolvedComplains" method="POST" action="unresolvedComplains.php">
					</form>
                        <table width='100%'>
							<tr><td width='40%'>
							<table>
							<?php
								$query = mysqli_query($db,"SELECT * FROM tblComplaints WHERE complainStatus = 0 ORDER BY complainDate") or die(mysqli_error($db));
								$i = 0;
