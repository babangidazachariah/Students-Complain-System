...
session_start();
	...
	if (isset($_POST['submit'])) {
		...
		$query = mysqli_query($db,"SELECT tblAdmin.*, tblInstitution.institutionName FROM tblAdmin, tblInstitution WHERE (tblAdmin.adminUsername = '$searchKey' OR tblAdmin.adminEmail = '$searchKey' OR tblAdmin.adminFirstName LIKE '%$searchKey%' OR tblAdmin.adminLastName LIKE '%$searchKey%') AND tblAdmin.adminSchool = tblInstitution.institutionID") or die(mysqli_error($db));
                ...
                <div class="col-sm-12 row text-justify">
                  <p></p>
                    <fieldset>
						...
						<font color='black' name='Arial' size='4'><b>REGISTERED ADMINS </b></font>
						</div>
							...
							<?php
								 if(!empty($search)){
									print($search);
										...
										$query = mysqli_query($db,"SELECT tblAdmin.*, tblInstitution.institutionName FROM tblAdmin, tblInstitution WHERE tblAdmin.adminSchool = tblInstitution.institutionID") or die(mysqli_error($db));
